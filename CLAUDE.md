...
`Membership.deactivatedAt` (null = active) is per-workspace, so a person can be active in one and deactivated in another. `GET /api/me` filters on `deactivatedAt: null` (in `listMyMemberships()`), so a deactivated workspace disappears from the client guard and the admin nav. Don't remove that filter.

**That filter alone is not enough, and relying on it was a real bug.** Better Auth authenticates the *account* and knows nothing about Membership, so a deactivated person still signed in successfully and landed in the app with `memberships: []` — signed in, with access to nothing. The rule is now enforced in `server/src/lib/workspace-access.ts`, which is the single definition of "can this account get in at all": **at least one active membership somewhere**. It has two callers, and both are needed:

- `auth.ts` hooks `databaseHooks.session.create.before` and throws `APIError('FORBIDDEN')` when there is none — hooked at session *creation*, not on the sign-in endpoint, so it also covers the magic link the Client role is getting later. It runs *after* the password is verified, which is why it may state the real reason instead of the deliberately-vague "Invalid email or password" — only the genuine account owner ever reaches it.
- `revokeSessionsWithoutAccess()` deletes their sessions when a write takes away their last active membership (`setMemberActive(false)`, and `deleteMember`'s membership-only branch — the account branch needs nothing, since `session` cascades from `user`). Without it "cannot log in" would coexist with "is still logged in" for up to seven days.

Because the question is about the account rather than one workspace, someone deactivated in workspace A but active in B signs in fine and simply stops seeing A. Signup is unaffected: it issues its session through `auth.api.signInEmail` after `createWorkspaceWithAdmin` has committed the ADMIN membership.
...
**Being inside a `$transaction` is not enough to enforce the last-admin rule** — that was a real bug, and it is why `lockWorkspace()` (`SELECT … FOR UPDATE` on the workspace row) runs first in both `updateMemberRole` and `setMemberActive`. Prisma uses Postgres's default READ COMMITTED and `count()` takes no locks, so two transactions demoting *different* admins each still see the other as active and both commit, leaving zero admins — unrecoverable in-app, since every route then fails `requireWorkspaceRole('ADMIN')`. Reproduced at 7/8 concurrent attempts without the lock, 0/8 with it. **Any future check shaped like "count rows, then write" needs the same lock.** It lives in `server/src/lib/workspace-lock.ts` (with the `Tx` type) and is shared — `createTicket` takes it too, for "read the highest ticket number, then insert the next one", which is the same shape.

`deleteMember()` is irreversible and **not an unconditional `user.delete`**. One email can hold memberships in several workspaces, so erasing the User row on one workspace admin's say-so would destroy an account another tenant depends on — reaching outside the workspace the request is scoped to. It therefore deletes the membership, and deletes the `User` (cascading Session, Account, Membership) **only when that was the person's last membership**. The response says which happened (`{ deleted: 'account' | 'membership' }`) and the UI reports it. Every account this app creates today has exactly one membership, so the full wipe is the normal path; the other branch is what stops a cross-tenant deletion. Same guards as deactivation — no self-deletion, last active ADMIN must stay — behind the same `lockWorkspace()`, since "count the other admins, then write" is exactly the read-then-write shape READ COMMITTED does not make safe. **It does not touch their tickets** — see "Deleting a person must not delete their work" below.
...
Client side: `client/src/lib/auth-client.ts` exports `authClient`/`useSession`; `lib/signup.ts` calls the custom endpoint. Pages are `/signup` and `/login` (sharing the `AuthShell` frame), `/profile` and `/tickets` (+ `/tickets/:ticketId`) for any signed-in user, and the admin-only `/users`. Identity is read through the `['me']` TanStack Query (`lib/me.ts`), not `useSession`. `components/RequireAuth.tsx` gates `/profile` and the ticket pages on having a session at all; `components/RequireRole.tsx` is a pathless layout route gating `/users` on ADMIN — it **controls navigation only**; every endpoint behind a guarded page still needs its own `requireWorkspaceRole`. `hasRole()` currently means "holds the role in *any* workspace" because there is no active-workspace concept yet; `primaryMembership()` (the same limitation, named — it returns `memberships[0]`) is what the ticket pages use to get both a workspace id and the role held there, and `UsersPage` picks the first ADMIN membership's workspace for the same reason. All three need re-scoping when a user can meaningfully belong to several.

Open before deploying: `advanced.ipAddress.trustedProxies` on the Better Auth side still needs the real proxy topology.

## Tickets

**Everything ticket-facing lives in `server/src/modules/tickets/`** — `tickets.routes.ts`, `tickets.service.ts`, `tickets.schemas.ts`, and `tickets.policy.ts`. Mounted bare from `index.ts`, same as `usersRouter`. Routes, all under `/api/workspaces/:workspaceId/`: `GET|POST tickets`, `GET|PATCH|DELETE tickets/:ticketId`, `POST tickets/:ticketId/comments`, and `GET ticket-options` (assignees + categories for the form selects, in one request).

Models: `Ticket`, `TicketComment` (a comment, or an internal note when `internal` is true — one table, because they are one conversation), and `TicketEvent` (append-only activity history). `TicketStatus` is the fixed MVP set (REQUESTED → OPEN → RESOLVED → CLOSED, not renameable by anyone); `TicketPriority` has five levels **declared in ascending severity**, so `orderBy: { priority: 'desc' }` means "most urgent first" without a CASE expression — don't reorder the enum.

### Deleting a person must not delete their work

**This is the load-bearing rule of the whole module.** A ticket is the workspace's operational record, so it has to stay readable after the people named on it are gone. Every person on a ticket is therefore stored **twice**:

1. A **nullable FK with `onDelete: SetNull`** (`requesterId`, `assigneeId`, `TicketComment.authorId`, `TicketEvent.actorId`). Deleting the account clears the pointer and leaves the row otherwise untouched.
2. A **denormalised name/email snapshot** written at the time of the action (`requesterName`, `assigneeName`, `authorName`, `actorName`, …). The FK going null loses the identity entirely; the snapshot is what makes the record durable.

**Never `Cascade` from User to anything that records work.** Session/Account/Membership cascade because they *are* the account; a ticket is not. Cascading would let one admin's "delete this person" erase tickets other people are actively working. Anything added later that references `User` has to make this choice explicitly — the Prisma default for a *required* relation is `Restrict`, which would instead break `deleteMember` outright.

Reads go through **`personFrom()` in `tickets.service.ts` — the one place** the resolution happens: prefer the live relation while it exists (so a rename in `/profile` shows through), fall back to the snapshot when it is null. It returns `{ id, name, email, deleted }`, and `deleted` is stated rather than left for the client to infer. `components/tickets/PersonLabel.tsx` renders it, marking a deleted account in words — a stale name must not read as a current member. **Display from the snapshot, filter from the FK:** "assigned to me" and the assignee filter query `assigneeId`, so they correctly stop matching a deleted account instead of matching a stale string.

Verified against real Postgres: after `deleteMember` wipes an account, every ticket, comment and event row survives with its snapshot intact and only the FK nulled; the ticket then reads as unassigned to the filters while still showing who it *was* assigned to.

Deleting a **ticket** is the opposite and genuinely destructive: its comments and events cascade from it, because they describe only that ticket. Admin-only, behind a confirmation.

### Ticket authorization

`requireWorkspaceRole('ADMIN','MANAGER','STAFF','CLIENT')` — i.e. any active member — is still the tenant boundary on every ticket route. The four roles are listed rather than wildcarded so a role added to the enum has to be considered here instead of silently gaining access. Role differences *within* the workspace are then applied by **`tickets.policy.ts`**, one named predicate per rule from `project-scope.md` §1/§4:

| | sees | create | edit subject/desc/category | status | priority | assign | internal notes | delete |
|---|---|---|---|---|---|---|---|---|
| ADMIN | all | ✓ | ✓ | ✓ | **✓ (only role)** | ✓ | ✓ | ✓ |
| MANAGER | all | ✓ | ✓ | ✓ | ✗ | ✓ | ✓ | ✗ |
| STAFF | **assigned to them only** | ✗ | ✗ | own assigned only | ✗ | ✗ | ✓ | ✗ |
| CLIENT | requested by them | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ |

Visibility is `ticketScopeFor(role)` → `'all' | 'assigned' | 'requested'`, turned into a `where` by `visibilityFilter()`. It narrows the *query*, never the results: filtering afterwards would still load another person's ticket into memory and would leak its existence through the pagination total. Internal notes are excluded from the query for a Client, so those rows are never loaded for them. A ticket a caller may not see returns **404, not 403** — a 403 would confirm the id exists.

**Visibility and the user's filters are combined with `AND`, not spread into one object.** A Staff member's visibility clause *is* `assigneeId = me`, and `?assigneeId=<someone-else>` is also an `assigneeId` key — spreading let the query string overwrite the permission check and hand them another person's queue. Under `AND` both must hold, so that request correctly returns nothing. Any future scope whose key collides with a filter key has the same hazard.

**Staff cannot create tickets, and that is required by the scope rule rather than a separate decision.** Staff see only tickets *assigned* to them; raising one makes them the requester, not the assignee, so it would vanish on creation — and `createTicket` re-reads through the caller's own visibility, so the request would fail outright. To let staff raise tickets, widen `ticketScopeFor` to "assigned to me OR raised by me"; do not re-enable `canCreateTicket` alone.

Priority override being Admin-only is stated twice in the scope doc — not an oversight to be widened into "leads".

`client/src/lib/ticket-policy.ts` mirrors this table to decide what to render. **It is not authorization** — every rule is enforced again server-side; the copy only stops the UI offering a control that would come back 403. If a rule changes, change the server first.

Two scope items are deliberately unimplemented and flagged at their rule: the per-client "may raise tickets" grant (no column on Membership yet, so `canCreateTicket` refuses CLIENT — which also means a CLIENT currently sees an empty queue, since they cannot become a requester), and automatic routing rules. Also not built, all blocked on infrastructure from later phases: attachments/deliverables (R2), AI categorisation and initial priority (Anthropic API — priority defaults to NORMAL), SLA timeframes and auto-escalation (BullMQ + Redis), email-to-ticket (SendGrid), the dynamic form builder, and bulk actions.

`createTicket` allocates a per-workspace `number` under `lockWorkspace()` — "read the highest, insert the next" is the read-then-write shape READ COMMITTED does not make safe, and the `@@unique([workspaceId, number])` would turn the loser of a race into a 500. This serialises ticket creation per workspace; if that ever matters, the fix is a per-workspace sequence, not dropping the guarantee. Verified: 8 concurrent creates get 8 gapless distinct numbers.

Client query keys: `['tickets', workspaceId, filters]` (the filter object is part of the key, so each combination caches separately), `['ticket', workspaceId, ticketId]`, `['ticket-options', workspaceId]`. A write invalidates both the ticket and the list — status, priority and assignee are columns in the list too.
...
- **Validation: zod on both sides.** No hand-rolled checks anywhere. Enums come from the Prisma-generated const objects via `z.enum(Role)` / `z.enum(TicketStatus)`, so adding a value needs no second list. On the server every route parses its body with `parseBody(schema, req.body)` and renders failures with `validationErrorBody(err)` — both from `server/src/lib/validation.ts` — which is what keeps the `{ error, fields? }` envelope identical across endpoints. `parseBody` normalises a non-object body (`null`, a string, an array — all valid JSON that `express.json()` passes through) to `{}` so the caller gets "this field is missing" instead of zod's internal "expected object, received null". `validationErrorBody` takes one message per field and prefers field issues over object-level ones, so a schema-level `.refine` (like `updateMemberSchema`'s "Nothing to update") never masks a specific field error. Field schemas shared by more than one route — `emailField`, `personNameField` — live in that same file; route-specific ones live with their module (`modules/users/users.schemas.ts`, `modules/tickets/tickets.schemas.ts`). Query-string params get the same treatment, not looser — `listTicketsQuerySchema` parses `req.query`, so an out-of-range `perPage` is a 400 rather than an unbounded query. **Write every message for an API client, never leaving zod's default**: a raw "Too big: expected number to be <=100" describes the schema instead of the request and breaks the envelope's consistency. Roles come from `z.enum(Role)` against the Prisma-generated const object, so adding a role to the schema needs no second list. Note zod checks *accumulate*: appending `.min(1, …)` to a shared field adds a second check rather than replacing the first, so a route needing its own message spells the field out.
- **Forms: react-hook-form + `zodResolver`, for every form, no exceptions.** This is the standard for anything new — signup, login, add member, profile, change password, new ticket, edit ticket and the comment box all follow it, and a new form that doesn't is the thing to fix. Concretely:
  - **No `useState` form state and no hand-rolled validation.** If a field needs checking, the check belongs in the schema.
  - **The schema lives in `client/src/lib/schemas.ts`** and is the single source of truth — value types are inferred with `z.infer`, so never hand-write a parallel input type.
  - **Mirror it server-side** in that route's `*.schemas.ts` (see the Validation bullet). The client copy is for fast feedback; the server is the authority, and the two drifting is a bug.
  - **`mutate`, not `mutateAsync`, inside `handleSubmit`.** A rejected `mutateAsync` propagates back out of react-hook-form with nothing awaiting it — an unhandled rejection on every failed submit. Render the failure from `mutation.error` instead.
  - **Push server field errors onto inputs with `setError`** in a `useEffect` on `mutation.error`, and show anything without a field as a form-level `role="alert"`. `AddMemberDialog` and `NewTicketDialog` are the worked examples.
  - **`values:` rather than `defaultValues:`** when the form edits something that can change underneath it (`ProfilePage`, `EditTicketDialog`), so it re-syncs instead of showing what it first mounted with.
  - Zod v4 API: `z.email()` is top-level (not `z.string().email()`) and custom messages use `{ error: '…' }`, not the deprecated `{ message: '…' }`.
  - A Base UI `Select` needs a string value, so "none" is carried by a sentinel (`''`, or `'__any__'` in the filter bar) and converted at the boundary — `null` for the API, which distinguishes "clear this" from an absent "leave it alone".
