...
`Membership.deactivatedAt` (null = active) is per-workspace, so a person can be active in one and deactivated in another. **`GET /api/me` filters on `deactivatedAt: null`, and that is what makes deactivation take effect** — sessions are not workspace-scoped, so the cookie stays valid, but the workspace disappears from `/api/me` and the client guard plus admin nav go with it. Don't remove that filter.

Workspace membership CRUD lives in `server/src/services/members.ts` behind `/api/workspaces/:workspaceId/users` (GET/POST/PATCH). Two invariants hold: a workspace must always keep at least one active ADMIN, and nobody can deactivate themselves.
